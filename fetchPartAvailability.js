...
const OUTPUT_FILE = path.join(DATA_DIR, `part-availability-${env}.json`)
...
async function fetchPartAvailability() {
  ...
  for (const part of parts) {
    ...
    console.log(`Fetching availability ${index}/${parts.length} | ${partNumber}`)
  ...
  console.log(`All part availability saved to ${OUTPUT_FILE}`)
}

fetchPartAvailability().catch(error => {
  console.error("Error while fetching part availability")
