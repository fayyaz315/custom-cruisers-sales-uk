...
const EU_LOCATION_ID = process.env.EU_LOCATION_ID
...
async function fetchShopifyVariant(sku) {
  try {
    ...
    if (response.data.errors) {
      console.log(`❌ SHOPIFY GRAPHQL ERROR | ${sku}`)
  ...
  try {
    ...
    const variables = {
      input: {
        name: "available",
        reason: "correction",
        referenceDocumentUri: "logistics://parts-europe/inventory-sync",
        quantities: [
          {
            inventoryItemId,
...
async function processSku(sku, index, total) {
  const partDetail = await fetchPartDetail(sku)

  if (!partDetail) {
    console.log(`❌ [${index}/${total}] ${sku} | PARTS API FAILED`)
    return
  }

  const euQuantity = partDetail.eu_availability || 0
  const usQuantity = partDetail.us_availability || 0

  const shopifyVariant = await fetchShopifyVariant(sku)

  if (!shopifyVariant) {
    console.log(`⚠️ [${index}/${total}] ${sku} | SHOPIFY NOT FOUND`)
    return
  }
  ...
  if (updated) {
    ...
    console.log(`❌ [${index}/${total}] ${sku} | UPDATE FAILED`)
  }
...
async function startInventoryLoop() {
  while (true) {
    try {
      console.log("\n🌍 ===============================================")
      console.log("🚀 STARTING INVENTORY SYNC LOOP")
      console.log("🌍 ===============================================\n")

      const skus = await fetchAvailabilityUpdates()

      console.log(`📦 TOTAL CHANGED SKUS: ${skus.length}\n`)

      let processed = 0

      for (const sku of skus) {
        processed++
        await processSku(sku, processed, skus.length)
      }

      console.log("\n✅ ===============================================")
      console.log("🎉 INVENTORY LOOP COMPLETED")
      console.log("✅ ===============================================\n")
