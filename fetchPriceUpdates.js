...
const OUTPUT_FILE = path.join(DATA_DIR, `price-updates-${env}.json`)
...
async function fetchPriceUpdates() {
  ...
  console.log("Fetching price updates...")

  while (hasNextPage) {
    const response = await axios.get(
      `${API_BASE_URL}/v1/parts/price-changes`,
      {
        headers: {
  ...
  console.log(`All price updates saved to ${OUTPUT_FILE}`)
}

fetchPriceUpdates().catch(error => {
  console.error("Error while fetching price updates")
